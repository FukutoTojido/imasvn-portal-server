...
import { privillage, token, uac } from "../middleware";
...
const archives = new Elysia()
	.group("/:slug/archives", (app) =>
		app
			.group("", (app) =>
				app
					.use(privillage)
			...
			.group("", (app) =>
				app.use(uac).get(
					"/:broadcast_id",
					async ({ params: { broadcast_id: id, slug }, status }) => {
						try {
							if (typeof id === "number") {
								const [entry] = await getConnection().query(
									"SELECT * FROM `live_archives` WHERE id=?",
							...
							if (typeof id === "string") {
								const [entry] = await getConnection().query(
									"SELECT * FROM `live_archives` WHERE broadcast_slug=? AND event_id=? AND archive=TRUE",
									[id, slug],
								);
					...
					},
					{
						params: t.Object({
							slug: t.String(),
							broadcast_id: t.Union([t.Number(), t.String()]),
						}),
					},
				),
			)
	...
	.group("/:slug/lives", (app) =>
		app.get(
			"/:broadcast_slug",
			async ({ params: { broadcast_slug: id, slug }, status }) => {
				try {
					const [entry] = await getConnection().query(
						"SELECT * FROM `live_archives` WHERE event_id=? AND broadcast_slug=? AND archive=FALSE",
						[slug, id],
					);
